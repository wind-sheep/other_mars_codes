...
# Replace with the class ID of "Soda" in your dataset
SODA_CLASS_ID = 0  # Update this based on your YOLO class mapping
...
def image_callback(msg):
    ...
    for result in results:
        ...
        for box in boxes:
            ...
            if class_id == SODA_CLASS_ID:  # Check if it's "Soda"
                ...
                label = f"Soda ({last_x_center}, {last_y_center})"
                cv2.putText(frame, label, (last_x_center + 5, last_y_center - 5),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)

                rospy.loginfo(f"Detected Soda: x_center={last_x_center}, y_center={last_y_center}")
...
def main():
    ...
    if last_x_center is not None and last_y_center is not None and last_depth is not None:
        print("\nLast detected Soda coordinates and depth:")
